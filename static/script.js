  ...
  if( params && params.command === 'openFile' ){
      ...
      api.fs( params.data, function( error, structure ){
        ...
        structure.getFormats( function( error, formats ){
          ...
          var dimensions = structure.formats.original.metadata ? structure.formats.original.metadata.pdf.pageSize.split(' ') : [ 630, 0, 891 ];
...
var _loadPdf = function( file ){
  ...
  if( file.mime === 'application/pdf' ){
    $('iframe').attr( 'src', 'https://beta.inevio.com/app/6/pdfjs/web/viewer.html?file=' + file.formats.original.url );
  }else if( file.formats['application/pdf'] ){
    $('iframe').attr( 'src', 'https://beta.inevio.com/app/6/pdfjs/web/viewer.html?file=' + file.formats['application/pdf'].url );
  }else{
